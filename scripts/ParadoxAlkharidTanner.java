...
public class ParadoxAlkharidTanner extends Script implements PaintListener, ServerMessageListener {
	...
	************/
	...
	RSTile[] TannerToBank = new RSTile[] { new RSTile(3282, 3181), new RSTile(3278, 3170), new RSTile(3270, 3166) };
    ...
    public boolean onStart(Map<String, String> args) {
    	...
    	if(isLoggedIn()) {
			...
			if(!inventoryContains(money)) {
				...
				else if(args.get("typeOfHides").equals("Cowhides (Hard Leather)")) {
					uth = utcow;
					ttc = tcowh;
	...
	public void getVersionOnline() {
        ...
        try {
			...
			if(version == getClass().getAnnotation(ScriptManifest.class).version()) {
				...
				log("There is a new update available. Please check the forums or download from link below.");
				log("http://paradox-projects.tk/");
			}
        }
		catch (Exception e) { log("Could not check for updates."); }
		wait(100);
		action = prevAction;
		return;
    }
	...
	private boolean walkPath2(final RSTile[] path, final int randX, final int randY) {
		while(distanceTo(path[path.length - 1]) > 4) {
			if(!walkPathMM(path, randX, randY) || !waitToMove(random(1500, 3000))) {
    ...
    public void onRepaint(Graphics g) {
		...
		if(debugPaint == true) {
			...
			g.drawString(getClass().getAnnotation(ScriptManifest.class).name(), 355, 25);
			g.setColor(Color.WHITE);
			g.setFont(new Font("Trebuchet MS", Font.PLAIN, 12));
			g.drawString("Version: " + getClass().getAnnotation(ScriptManifest.class).version(), 355, 40);
			g.drawString("Runtime: " + hours + ":" + minutes + ":" + seconds, 355, 55);
			...
			if(profitMade > 10000) {
				g.drawString("Profit: " + round((profitMade/1000), 2) + "K", 355, 130);
			}
			else {
				g.drawString("Profit: " + round(profitMade, 1) + " GP", 355, 130);
			}
			if(profitH > 10000) {
				g.drawString("Profit / hour: " + round((profitH/1000), 2) + "K/h", 355, 145);
			}
			else {
				g.drawString("Profit / hour: " + round(profitH, 1) + " GP/h", 355, 145);
			}
