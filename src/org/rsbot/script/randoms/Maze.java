...
public class Maze extends Random {
	...
	public boolean activateCondition() {
		if (isLoggedIn() && ((getNearestObjectByID(3626) != null) || (getNearestObjectByID(3649) != null))) {
	...
	public int loop() {
		...
		if (getMyPlayer().getLocation().equals(new RSTile(2911, 4576))) {
			if (getMyPlayer().getAnimation() == -1) {
				atTile(new RSTile(2912, 4576), "Touch");
		...
		if ((walkToTile != null) && (distanceTo(walkToTile) >= 3)) {
			walkTo(new RSTile(walkToTile.getX(), walkToTile.getY()));
			return random(500,600);
		}
		if ((walkToTile != null) && (distanceTo(walkToTile) <= 3)) {
			if ((doorDir != 'a') && !getMyPlayer().isMoving()) {
				if (((getCameraAngle() - turnCameraTo()) < 30) || ((getCameraAngle() - turnCameraTo()) > 30)) {
					setCameraRotation(turnCameraTo());
				}
	...
	public RSTile tileAfterDoor() {
		...
		switch (doorD) {
			case 'n':
				return new RSTile(wallTile.getX(), wallTile.getY() + 1);
			case 'w':
				return new RSTile(wallTile.getX() - 1, wallTile.getY());
			case 'e':
				return new RSTile(wallTile.getX() + 1, wallTile.getY());
			case 's':
				return new RSTile(wallTile.getX(), wallTile.getY() - 1);
